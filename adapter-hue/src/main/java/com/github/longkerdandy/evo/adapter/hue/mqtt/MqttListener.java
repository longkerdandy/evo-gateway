...
import com.github.longkerdandy.evo.api.message.ActionMessage;
...
public class MqttListener implements MqttCallback {
    ...
    protected MqttClient client;            // Paho MQTT Client
    ...
    public void subscribe(String topic, int qos) throws MqttException {
        ...
        logger.debug("Successful subscribe to topic {}", topic);
    }

    @Override
    public void connectionLost(Throwable cause) {
        logger.trace("Received connectionLost event {}", ExceptionUtils.getMessage(cause));
    }
    ...
    public void messageArrived(String topic, MqttMessage mqttMessage) throws Exception {
        ...
        logger.debug("Received messageArrived event on topic {}", topic);
        try {
            JavaType type = JsonUtils.ObjectMapper.getTypeFactory().constructParametricType(Message.class, ActionMessage.class);
            Message<ActionMessage> message = JsonUtils.ObjectMapper.readValue(mqttMessage.getPayload(), type);
            this.hueListener.doAction(message);
        } catch (IOException e) {
            logger.warn("Json parse exception: {}", ExceptionUtils.getMessage(e));
        }
    }

    @Override
    public void deliveryComplete(IMqttDeliveryToken iMqttDeliveryToken) {
        logger.trace("Received deliveryComplete event");
    }
}
